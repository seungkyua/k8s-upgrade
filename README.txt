...
선택 옵션
  ...
  --ignore-preflight-errors <list>
                                kubeadm upgrade plan/apply/node 에 전달할
                                --ignore-preflight-errors 값 (콤마로 구분, 기본값: CreateJob)
                                kubeadm 이 업그레이드 전 kube-system 에 만드는 헬스체크용
                                Job 이 15초 안에 끝나지 못해 FATAL 로 실패하는 경우가 흔해
                                기본으로 무시하도록 설정되어 있습니다. 완전히 끄려면
                                --ignore-preflight-errors "" 로 빈 값을 지정하세요.
...
3. kubeadm 패키지 unhold -> 설치 -> hold
   설치 직후 /usr/bin/kubeadm (apt 가 설치하는 경로) 이 실제로 방금 설치한
   버전을 보고하는지 확인합니다. PATH 상에 다른 kubeadm 바이너리(예: Kubespray 로
   설치된 /usr/local/bin/kubeadm)가 먼저 잡혀 있으면 여기서 바로 에러로 종료됩니다.
4. (컨트롤 플레인 노드만) kubeadm upgrade plan 출력
5. 역할별 업그레이드 적용 (모두 --ignore-preflight-errors=CreateJob 기본 적용)
   - primary-control  : kube-apiserver 종료 후 재기동될 때까지 대기(최대 180초) -> kubeadm upgrade apply v<ver> --certificate-renewal=false
   - secondary-control: kube-apiserver 종료 후 재기동될 때까지 대기(최대 180초) -> kubeadm upgrade node --certificate-renewal=false
   - worker           : kubeadm upgrade node --certificate-renewal=false
   (kube-apiserver 가 180초 안에 https://127.0.0.1:6443/livez 에 응답하지 않으면
    스크립트를 종료합니다)
...
- containerd 패키지 자동 감지는 노드에 containerd 또는 containerd.io 중 하나가 이미
  ...
  지정하세요.
- 패키지 설치(apt-get install) 시 needrestart 가 자동으로 관련 서비스(특히 containerd)를
  재시작하지 않도록 NEEDRESTART_MODE=l 을 적용합니다. needrestart 가 containerd 를
  임의로 재시작하면 노드의 모든 컨테이너(etcd, kube-apiserver 포함)가 함께 내려가
  업그레이드 도중 "connection refused" 오류가 발생할 수 있기 때문입니다.
- kubeadm 은 업그레이드 전 kube-system 네임스페이스에 헬스체크용 Job(CreateJob)을
  만들어 15초 안에 끝나는지 확인하는데, 클러스터가 정상이어도 스케줄링/이미지 pull
  지연으로 이 체크가 자주 FATAL 로 실패합니다. 기본적으로 --ignore-preflight-errors=CreateJob
  으로 무시하도록 처리되어 있습니다.
- kubeadm 관련 명령은 모두 절대경로(/usr/bin/kubeadm)로 실행합니다. Kubespray 등으로
  구축된 클러스터는 /usr/local/bin 에 별도로 kubeadm 바이너리를 배치해 둔 경우가 있는데,
  sudo 의 기본 secure_path 는 /usr/local/bin 이 /usr/bin 보다 우선이라 "sudo kubeadm" 만
  실행하면 apt 로 새로 설치한 바이너리가 아니라 그 예전 바이너리가 실행되어 버전
  불일치 오류("kubeadm version: vX.Y.Z" 가 목표 버전과 다르게 나오는 등)가 발생할 수
  있습니다. 절대경로 지정으로 이를 방지합니다.
